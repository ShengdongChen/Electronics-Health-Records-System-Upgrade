...
public class APILabProcedureController extends APIController {
     ...
     */
    @PreAuthorize ( "hasAnyRole('ROLE_PHARMACIST', 'ROLE_HCP', 'ROLE_OD', 'ROLE_OPH', 'ROLE_LABTECH', 'ROLE_VIROLOGIST')" )
    ...
    public ResponseEntity getLabProcedures () {
        ...
        for ( final LabProcedure proc : procs ) {
            User.unlinkCircularReferences( proc.getPatient() );
            proc.getVisit().setLabProcedures( null );
        }
    ...
    public ResponseEntity getProceduresForTech ( @PathVariable ( "techId" ) final String techId ) {
        ...
        if ( tech != null && tech.getRole().equals( Role.ROLE_LABTECH ) ) {
            ...
            for ( final LabProcedure proc : procs ) {
                User.unlinkCircularReferences( proc.getPatient() );
                User.unlinkCircularReferences( proc.getAssignedTech() );
                proc.setVisit( null );
            }
    ...
    public ResponseEntity getProceduresForOfficeVisit ( @PathVariable ( "id" ) final Long id ) {
        ...
        final boolean isPHA = SecurityContextHolder.getContext().getAuthentication().getAuthorities()
                .contains( new SimpleGrantedAuthority( "ROLE_PHARMACIST" ) );
        if ( isHCP || isOD || isOPH || isPHA ) {
            if ( ov != null ) {
                logCode = TransactionType.HCP_VIEW_PROCS;
    ...
    @PreAuthorize ( "hasAnyRole('ROLE_PHARMACIST', 'ROLE_HCP', 'ROLE_OD', 'ROLE_OPH', 'ROLE_VIROLOGIST', 'ROLE_LABTECH')" )
    ...
    public ResponseEntity updateLabProcedure ( @PathVariable final Long id, @RequestBody final LabProcedureForm form ) {
        try {
            ...
            User.unlinkCircularReferences( lp.getAssignedTech() );
            User.unlinkCircularReferences( lp.getPatient() );
            lp.setVisit( null );
    ...
    private Set<LabProcedureForm> createResponseForms ( final List<LabProcedure> procs ) {
        final Set<LabProcedureForm> response = new HashSet<>();
        for ( final LabProcedure proc : procs ) {
            response.add( new LabProcedureForm( proc ) );
        }
        return response;
    }
