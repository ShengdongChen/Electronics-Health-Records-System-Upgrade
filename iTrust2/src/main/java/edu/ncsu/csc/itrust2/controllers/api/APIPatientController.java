...
public class APIPatientController extends APIController {
    ...
    public Set<PatientForm> getPatients () {
        final List<Patient> patients = Patient.getPatients();
        final Set<PatientForm> response = createResponseForms( patients );
        return response;
    }
    ...
    public ResponseEntity getPatient () {
        ...
        if ( patient == null ) {
            ...
            return new ResponseEntity( new PatientForm( patient ), HttpStatus.OK );
        }
    }
    ...
    public ResponseEntity createPatient ( @RequestBody final PatientForm patientF ) {
        try {
            ...
            System.out.println( "Could not create " + patientF.getSelf() + " " + patientF.getEmail() + " because of "
                    + e.getMessage() );
            return new ResponseEntity(
    ...
    public ResponseEntity updatePatient ( @PathVariable final String id, @RequestBody final PatientForm patientF ) {
        ...
        try {
            if ( ( !auth.getAuthorities().contains( new SimpleGrantedAuthority( "ROLE_HCP" ) )
                    ...
                    && !auth.getAuthorities().contains( new SimpleGrantedAuthority( "ROLE_PHARMACIST" ) ) )
     ...
     * @return response
     */
    @PutMapping ( BASE_PATH + "/patient/pharmacy/{pharmacy}" )
    @PreAuthorize ( "hasRole( 'ROLE_PATIENT')" )
    public ResponseEntity setDefaultPharmacy ( @PathVariable final String pharmacy ) {
        ...
        final User self = User.getByName( LoggerUtil.currentUser() );
        Patient patient = Patient.getByName( self.getUsername() );
        final PatientForm form = new PatientForm( patient );

        try {
            patient.setDefaultPharmacy( Pharmacy.getByName( pharmacy ) );
            ...
            LoggerUtil.log( TransactionType.PATIENT_DEFAULT_PHARMACY, LoggerUtil.currentUser(),
                    "User with username " + patient.getSelf().getUsername() + "updated their default pharmacy" );
            ...
            return new ResponseEntity(
                    errorResponse( "Could not set a new default pharmacy for " + patient.toString() ),
                    HttpStatus.BAD_REQUEST );
        }
    }

    private Set<PatientForm> createResponseForms ( final List<Patient> patients ) {
        final Set<PatientForm> patientForms = new HashSet<>();
        for ( final Patient patient : patients ) {
            patientForms.add( new PatientForm( patient ) );
        }
        return patientForms;
    }
