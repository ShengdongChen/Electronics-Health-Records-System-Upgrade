...
public class PatientController {
    ...
    /**
     * Provides the page for a User to view and edit their default pharmacy
     *
     * @param model
     *            The data for the front end
     * @return The page to show the user so they can edit demographics
     */
    @GetMapping ( value = "patient/defaultPharmacy/editDefaultPharmacy" )
    @PreAuthorize ( "hasRole('ROLE_PATIENT')" )
    public String viewDefaultPharmacy ( final Model model ) {
        final User self = User.getByName( LoggerUtil.currentUser() );
        LoggerUtil.log( TransactionType.PATIENT_DEFAULT_PHARMACY, self );
        return "/patient/defaultPharmacy/editDefaultPharmacy";
    }
