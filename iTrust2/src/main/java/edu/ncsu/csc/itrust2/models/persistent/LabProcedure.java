...
public class LabProcedure extends DomainObject<LabProcedure> {
    ...
    public static void unlinkCircularReferences ( LabProcedure proc ) {
        if ( proc == null ) {
            return;
        }

        if ( proc.getPatient() != null ) {
            User.unlinkCircularReferences( proc.getPatient() );
        }
        if ( proc.getVisit() != null ) {
            if ( !proc.getVisit().getPrescriptions().isEmpty() ) {
                for ( final Prescription p : proc.getVisit().getPrescriptions() ) {
                    Prescription.unlinkCircularReferences( p );
                }
            }
            if ( proc.getVisit().getBasicHealthMetrics() != null ) {
                User.unlinkCircularReferences( proc.getVisit().getBasicHealthMetrics().getPatient() );
            }
        }
