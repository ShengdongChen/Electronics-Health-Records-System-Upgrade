...
public class User extends DomainObject<User> implements Serializable {
     ...
     * @param phar
     *            pharmacy of the user(only specified for pharmacist)
     */
    public User ( final String username, final String password, final Role role, final Integer enabled,
            final Pharmacy phar ) {
        if ( !Role.ROLE_PHARMACIST.equals( role ) && phar != null ) {
            throw new IllegalArgumentException( "Do not assign pharmacy to users other than pharmacist!" );
        }

        if ( role.equals( Role.ROLE_PHARMACIST ) && phar == null ) {
            throw new IllegalArgumentException( "Pharmacy must be assigned to a pharmacist!" );
        }
    ...
    public User ( final String username, final String password, final Role role, final Integer enabled ) {
        this( username, password, role, enabled, null );
    ...
    public User ( final UserForm form ) {
        ...
        if ( Role.valueOf( form.getRole() ).equals( Role.ROLE_PHARMACIST ) ) {
    ...
    /**
     * the pharmacy of the user
     */
    @ManyToOne
    @JoinColumn ( name = "pharmacy_id", columnDefinition = "varchar(100)" )
    ...
    public void setPharmacy ( final Pharmacy phar ) {
        this.phar = phar;
    }
    ...
    public boolean isDoctor () {
        return ( role == Role.ROLE_PHARMACIST || role == Role.ROLE_HCP || role == Role.ROLE_OD || role == Role.ROLE_OPH
                || role == Role.ROLE_VIROLOGIST );
    }

    /**
     * Helper functions to help the API to avoid StackOverFlows from circular
     * references
     *
     * @param user
     *            The user to remove circular references from
     */
    public static void unlinkCircularReferences ( final User user ) {
        if ( user == null ) {
            return;
        }

        final Pharmacy pharmacy = user.getPharmacy();
        if ( pharmacy != null ) {
            for ( final Prescription p : pharmacy.getPrescriptions() ) {
                Prescription.unlinkCircularReferences( p );
            }
            pharmacy.setPatients( null );
        }
    }
     ...
     *
     * @param users
     *            The list of users to remove circular references from
     * @return The updated list of users (not strictly necessary)
     */
    public static List<User> unlinkCircularReferences ( final List<User> users ) {
        if ( users != null ) {
            for ( final User u : users ) {
                unlinkCircularReferences( u );
            }
        }
        return users;
    }
