...
public class APIDiagnosisTest {
    ...
    public void testDiagnoses () throws Exception {
        ...
        GeneralCheckupForm visit = null;

        try {
            visit = gson.fromJson( content, GeneralCheckupForm.class );
        }
        ...
        for ( final Diagnosis dd : dlist ) {
            if ( dd.getCode().equals( d.getCode() ) && dd.getNote().equals( d.getNote() )
                    && dd.getVisit().getId().equals( Long.valueOf( visit.getId() ) ) ) {
                flag = true;
                d.setId( dd.getId() );
