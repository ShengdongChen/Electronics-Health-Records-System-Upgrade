...
public class APIPatientTest {
    ...
    /**
     * Tests the set default pharmacy functionality
     *
     * @throws Exception
    ...
    @Test
    @WithMockUser ( username = "antti", roles = { "PATIENT" } )
    public void testDefaultPharmacy () throws Exception {
        final PharmacyForm pharma = new PharmacyForm();
        pharma.setName( "generic" );
        pharma.setAddress( "123 side street" );
        pharma.setState( "NC" );
        pharma.setZip( "12345" );
        final Pharmacy p = new Pharmacy( pharma );
        p.save();
        ...
        final Patient pat = new Patient( patient );
        pat.save();
        assertNull( pat.getDefaultPharmacy() );

        patient.setDefaultPharmacy( pharma.getName() );

        mvc.perform( put( "/api/v1/patient/pharmacy/generic" ).contentType( MediaType.APPLICATION_JSON ) )
                .andExpect( status().isOk() );

        final Patient editedPatient = Patient.getByName( "antti" );

        assertEquals( editedPatient.getDefaultPharmacy().getName(), p.getName() );
    }
